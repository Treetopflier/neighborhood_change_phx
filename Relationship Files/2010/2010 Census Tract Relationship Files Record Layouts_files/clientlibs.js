...
var accordionChevronUp = "/etc/designs/census/images/upward-chevron.svg";
var accordionChevronDown = "/etc/designs/census/images/downward-chevron.svg";
var mobileWidth = 576;
...
var CensusAccordion = (function($, _, digitalData) {
    ...
    var initAccordion = function(id) {
    	...
    	$accordion.find('.data-uscb-accordion-expand-all').click(function () {
            ...
            accordion.accordionObj.isOpenCount = $(".uscb-accordion").length;
    	...
    	$accordion.find('.data-uscb-accordion-collapse-all').click(function () {
        	var accordion = findParentAccordion($(this));
        ...
        function fireAnalytics($element, additional) {
        	...
        	var mainTitle = accordion.$accordion.find(".data-uscb-accordion-main-title").first().text();
        	var headerText = accordion.$accordion.find(".data-uscb-accordion-header").first().text();
        	
        	var sectionData = mainTitle + "-" + headerText + (additional ? additional : '');
        	
            digitalData.event.eventInfo.expandCollapseList.clickName = digitalData.page.pageInfo.pageName + '^' + sectionData;
	    ...
	    for (var i = 0; i < fData.length; i++) {
            ...
            if (fData[i].country_selector_type === "aemattachment") {
				listResult += "<td class='uscb-padding-T-20 uscb-padding-B-20' width='" + colWidth + "%'>";
                    ...
                    " onclick='buttonClick(this);' class='uscb-capitalize uscb-color-accent uscb-overflow-ellipsis'>";
    ...
    var initCarousel = function(id, items) {
        ...
        function updateMediaContainer($carousel, carouselObj) {
	        ...
	        if (carouselObj.currentObject.mediaType === 'image') {
				...
				mediaContainerMarkup += '<img src="' + carouselObj.currentObject.image + '" id="uscb-carousel-img-spec">';
							...
							if (ui.item.show_item == 'uscb-no-show-attachment') {
								...
								if (ui.item.open_new_page) {
									...
									$countrySelectorInput
											...
											.after(
													"<img class='uscb-country-item-close' src='/etc/designs/census/images/close-3-teal.svg' onClick='clearSelectionGeographicArea(\'" + countrySelectorWrapperDivId + "\');/>")
											.before(
								...
								$countrySelectorInput
										.val(
												ui.item.geographic_area_name
														...
														+ ui.item.file_size
														+ "]")
										.after("<span class='uscb-geographic-area-level'>" + ui.item.geographic_area_level + "</span>&nbsp;<img class='uscb-country-item-close' src='/etc/designs/census/images/close-3-teal.svg' onClick=\"clearSelectionGeographicArea('" + countrySelectorWrapperDivId + "');\"/>")
							...
							if (ui.item.show_item == 'uscb-no-show-attachment') {
								...
								if (ui.item.open_new_page) {
									...
									$stateListInput
											.val(ui.item.value)
											.after(
													"<img class='uscb-state-item-close' src='/etc/designs/census/images/close-3-teal.svg' onClick='clearSelectionStateList(\"" + stateListWrapperDivId + "\");'/>")
											.before(
													"<img class='uscb-input-box-icon "
															+ ui.item.icon_status
															+ "' src='"
...
function moveLanguageSelector(){
	if (Modernizr.mq('only screen and (min-width : 10px) and (max-width : 959px)')) {//was 576px
...
function adjustResponsiveIFrame(id, url) {
	var riObj = $("#" + id);
	if (riObj) {
		var containerWidth = riObj.parent().parent().width();		
